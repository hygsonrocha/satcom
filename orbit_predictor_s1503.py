...
Re = 6378.145  # radius of the Earth, in km
mu = 398601.8  # Kepler's constant, in km^3/s^2
we = 2 * np.pi / (24 * 3600)  # earth's average rotation rate, in rad/s
...
a = (hp + ha + 2 * Re) / 2  # semi-major axis, in km
e = (ha + Re - a) / a  # orbital eccentricity (e)
P = 2 * np.pi * np.sqrt(a ** 3 / mu)  # orbital period, in seconds
...
def plot_ground_tracks(theta_deg, phi_deg, planes=None, satellites=None, title="Satellite Ground Tracks"):
    ...
    Parameters:
        theta_deg (ndarray): Array of satellite latitudes in degrees, shape (num_planes * num_satellites_per_plane, num_timesteps).
        phi_deg (ndarray): Array of satellite longitudes in degrees, shape (num_planes * num_satellites_per_plane, num_timesteps).
...
OmegaG = (Omega0[:, None] + we * t)  # shape (Np*Nsp, len(t))
...
df = pd.DataFrame(data, columns=['time', 'plane', 'satellite', 'M_o', 'Omega_o', 'r', 'lat', 'lon', 'sx', 'sy', 'sz'])
